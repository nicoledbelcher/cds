...
**Note: you should only need to build if you're missing your desired build in [prebuilds](/apps/expo-app/prebuilds) or if there's been a recent native dependency upgrade.**
...
| android  | release - hermes      | `yarn nx run expo-app:build:android-release` |

**Note: If you run into errors when trying to prebuild, check out our [Help](/apps/expo-app/docs/help.md) page to debug.**
...
| android  | release - hermes      | `yarn nx run expo-app:launch:android-release` |
...
**Note: If you see `CommandError: No development build (com.anonymous.expo-app) for this project is installed. Please make and install a development build on the device first.` run `rm -rf apps/expo-app/ios apps/expo-app/.expo` and rerun the `start` script. See more debug gotchas [here](/apps/expo-app/docs/help.md)**
...
There are three core NX targets associated with Expo that we leverage to build and run expo-app. The various contexts can be summarized as debug and release modes for development, with release builds also serving as the basis for visual regression testing.

The three NX Targets (also declared in `/apps/expo-app/project.json`):

1. launch
2. start
3. build

`launch` and `build` call node scripts that live in the [scripts directory of expo-app](/apps/expo-app/scripts/). `start` runs `npx expo start` directly.
...
There are four relevant build variations associated with expo-app:
