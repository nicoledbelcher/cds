...
```shell
yarn workspace test-expo add expo@^<version on npm>
```
...
```shell
cd apps/test-expo && npx expo install --fix
```
...
4. Nuke your repo. Cached versions will be compiled in the expo build step and lead to version mismatches. `.nx`, `apps/test-expo/.expo`, `apps/test-expo/ios`, `apps/test-expo/android` should all be removed. Node modules should be removed because of version mismatches as well. Start fresh :)

```shell
cd ../../ && rm -rf apps/test-expo/ios apps/test-expo/android apps/test-expo/.expo && yarn clean && rm -rf node_modules
```
...
```shell
yarn nx run test-expo:build:ios-debug
yarn nx run test-expo:build:android-debug
...
[See our help docs](/apps/test-expo/docs/help.md)
