...
Some components expose static CSS classnames that you can use to target them with CSS. You can come this with data-attributes to customize styles based on state.
...
/* Target Button when it's in a loading state */
.cds-Button[data-loading] {
  opacity: 0.7;
}
```

Check a component's documentation page to see if it supports static classnames and what selectors are available.
