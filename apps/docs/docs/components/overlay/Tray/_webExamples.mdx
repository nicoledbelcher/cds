...
function BasicTray() {
  ...
  return (
    <VStack gap={2}>
      ...
      {visible && (
        <Tray title="Example Title" pin="right" onCloseComplete={handleClose}>
          {({ handleClose }) => (
            <VStack gap={2} justifyContent="space-between" flexGrow={1} paddingBottom={1.5}>
...
You can include a full-bleed custom image in the header by using negative margins to break out of the parent padding.
          ...
          title={
            <Box flexGrow={1}>
              <VStack gap={2} marginTop={-4}>
                <Box marginX={-4} marginRight={6}>
                  <img
                    src="https://cds.coinbase.com/img/card/content_card_rewards.png"
                    alt="Rewards"
                    style={{
                      width: '100%',
                      height: 220,
