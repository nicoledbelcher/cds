...
Charts have built in highlighting functionality which can be enabled by setting `enableHighlighting` to true. You can listen to value changes through `onHighlightChange`. Adding `Scrubber` to LineChart showcases the current highlighted position.

```jsx
function Interaction() {
  const [highlightedIndex, setHighlightedIndex] = useState<number | undefined>();

  const handleHighlightChange = useCallback((items) => {
    setHighlightedIndex(items[0]?.dataIndex ?? undefined);
  }, []);
...
function Performance() {
  ...
  return (
    <VStack gap={2} style={{ marginLeft: -8, marginRight: -8 }}>
      ...
      />
      <PerformanceChart onHighlightChange={handleHighlightChange} timePeriod={timePeriod} />
...
const PerformanceHeader = memo(
  ({
    ...
    const shownPosition =
      highlightedIndex !== undefined ? highlightedIndex : sparklineTimePeriodDataValues.length - 1;
...
const PerformanceChart = memo(
  ({
    ...
    timePeriod: TabValue;
    onHighlightChange: (items: Array<{ dataIndex: number | null; seriesId: string | null }>) => void;
  }) => {
    const theme = useTheme();
...
function BasicAccessible() {
  ...
  // Dynamic accessibility label provides specific position info
  const dynamicAccessibilityLabel = useCallback(
    (index: number) => {
  ...
  const accessibilityLabel = useMemo(() => {
    if (highlightedIndex !== undefined) {
      return dynamicAccessibilityLabel(highlightedIndex);
    }
    return chartAccessibilityLabel;
  }, [highlightedIndex, chartAccessibilityLabel, dynamicAccessibilityLabel]);
...
Using `legend` will add a default [Legend](/components/graphs/Legend) to your chart.
...
<LineChart
  ...
  yAxis={{
    showGrid: true,
    tickLabelFormatter: (value: number) => new Intl.NumberFormat('en-US', { maximumFractionDigits: 0 }).format(value),
  }}
/>
```
