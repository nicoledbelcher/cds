...
import type { ComponentTheme } from '../core/theme';
 ...
 *
 * @param componentName - The component key in ComponentTheme (e.g., 'Button')
 ...
 */
export const useComponentConfig = <K extends keyof ComponentTheme, P extends Record<string, any>>(
  componentName: K,
  localProps: P,
): P => {
  const store = useComponentConfigStore();

  const rawConfig = useStore(store, (state) => state[componentName]);
  const mergeFlag = useStore(store, (state) => state.mergeClassNameAndStyle);
