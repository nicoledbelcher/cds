...
describe('DatePicker', () => {
  ...
  it('applies custom dateInputStyle', () => {
    const customStyle = { marginBottom: 20 };
    render(<DatePickerExample dateInputStyle={customStyle} label="Date" />);

    const input = screen.getByTestId(testID);
    expect(input).toBeTruthy();
  });

  it('applies custom calendarStyle', async () => {
    const customStyle = { backgroundColor: 'red' };
    render(<DatePickerExample calendarStyle={customStyle} label="Date" />);
