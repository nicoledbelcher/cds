...
describe('Calendar', () => {
  ...
  it('does not call onPressDate when date buttons are disabled', () => {
    ...
    // Disabled dates are rendered as disabled buttons (via Tooltip triggerDisabled)
    const allButtons = screen.getAllByRole('button');
    const enabledDateButtons = allButtons.filter((button) => {
    ...
    // Only dates up to maxDate (July 10) should be enabled buttons
    expect(enabledDateButtons.length).toBeLessThan(31);
  });
  ...
  it('respects minDate and disables dates before it', () => {
    ...
    // Dates before July 10 should be disabled, not enabled
    expect(enabledDateButtons.length).toBeLessThan(31);
  });
  ...
  it('respects maxDate and disables dates after it', () => {
    ...
    // Dates after July 20 should be disabled, not enabled
    expect(enabledDateButtons.length).toBeLessThan(31);
  });
  ...
  it('respects disabledDates prop', () => {
    ...
    // July has 31 days, but 2 are disabled
    expect(enabledDateButtons.length).toBeLessThan(31);
  });
  ...
  it('respects disabledDates with date ranges', () => {
    ...
    // July 10-20 should be disabled (11 days), leaving 20 enabled
    expect(enabledDateButtons.length).toBe(20);
  });
